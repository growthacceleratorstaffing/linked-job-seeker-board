-- Create a function to validate if an email exists in Workable
...
-- Update the handle_new_user function to properly link to existing Workable users
...
BEGIN
    ...
    -- Check if this is the admin (first user or bart@growthaccelerator.nl)
    IF new.email = 'bart@growthaccelerator.nl' OR NOT EXISTS (SELECT 1 FROM public.user_roles WHERE role = 'admin') THEN
        ...
        ELSE
            -- Update existing workable_users record with the new user_id
    ...
    ELSE
        ...
        IF FOUND THEN
            ...
            -- Update the existing Workable user record with the new user_id
            UPDATE public.workable_users 
            SET user_id = new.id
            WHERE workable_email = new.email;
        ELSE
            -- This shouldn't happen if validation is working, but as fallback
